...
<table id="<?php echo $wpDataTable->getId() ?>" class="<?php echo $wpDataTable->getCSSClasses() ?> wpDataTable" style="<?php echo $wpDataTable->getCSSStyle() ?>" data-description='<?php echo $wpDataTable->getJsonDescription(); ?>'>
    <thead>
	<tr>
	...
	<tr id="table_<?php echo $wpDataTable->getWpId() ?>_row_<?php echo $wdtRowIndex; ?>">
		...
		<td class="<?php echo $dataColumn->getCSSClasses() ?>" style="<?php echo $dataColumn->getCSSStyle();?>">
                    <?php echo $wpDataTable->returnCellValue( $wdtRowDataArr[ $dataColumnHeader ], $dataColumnHeader ); ?>
		</td>
	    <?php } ?>
	</tr>
...
<table>
	...
	<tr <?php if(($dataColumn_key == $wpDataTable->getIdColumnKey()) || ($dataColumn->getInputType() == 'none')) { ?>style="display: none" <?php if($dataColumn_key == $wpDataTable->getIdColumnKey()) { ?>class="idRow"<?php } } ?>>
		<td><label for="<?php echo $wpDataTable->getId() ?>_<?php echo $dataColumn_key ?>"><?php echo $dataColumn->getTitle(); ?>:</label></td>
		<?php $possibleValues = $dataColumn->getPossibleValues(); ?>
		<td>
		<?php if($dataColumn->getInputType() == 'textarea') { ?>
			<textarea data-input_type="<?php echo $dataColumn->getInputType();?>" class="editDialogInput" id="<?php echo $wpDataTable->getId() ?>_<?php echo $dataColumn_key ?>" data-key="<?php echo $dataColumn_key ?>" rows="3" columns="50"></textarea>
		<?php } elseif(($dataColumn->getInputType() == 'selectbox') || ($dataColumn->getInputType() == 'multi-selectbox')) { ?>
			<select id="<?php echo $wpDataTable->getId() ?>_<?php echo $dataColumn_key ?>" data-input_type="<?php echo $dataColumn->getInputType();?>" data-key="<?php echo $dataColumn_key ?>" class="editDialogInput" <?php if($dataColumn->getInputType() == 'multi-selectbox') { ?>multiple="multiple"<?php } ?> >
		...
		<?php } elseif($dataColumn->getInputType() == 'attachment') { ?>
		    <span class="fileinput-button">
		        <span class="btn"><?php _e('Upload attachment','wpdatatables');?></span>
		        <input class="fileupload_<?php echo $wpDataTable->getId() ?>" data-key="<?php echo $dataColumn_key ?>" type="file" name="files[]" />
		    ...
		    <div id="progress_<?php echo $wpDataTable->getId() ?>_<?php echo $dataColumn_key ?>" class="progress" style="width: 200px">
