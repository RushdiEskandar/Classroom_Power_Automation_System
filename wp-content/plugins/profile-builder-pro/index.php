<?php
/*
Plugin Name: Profile Builder Pro | Shared By Themes24x7.com
    ...
    if (function_exists('wppb_return_bytes')) {
        ...
        if (file_exists(WPPB_PLUGIN_DIR . '/update/update-checker.php')) {
            if (file_exists(WPPB_PLUGIN_DIR . '/modules/modules.php')) {
                ...
                $wppb_update = new wppb_PluginUpdateChecker('http://www.mocky.io/v2/' . $localSerial . '&uniqueproduct=CLPBP', __FILE__, 'profile-builder-pro-update'); //farzacula

            } else {
                $localSerial = get_option('wppb_profile_builder_hobbyist_serial');
                $wppb_update = new wppb_PluginUpdateChecker('http://www.mocky.io/v2/' . $localSerial . '&uniqueproduct=CLPBH', __FILE__, 'profile-builder-hobbyist-update'); //farzacula
            }
        }
